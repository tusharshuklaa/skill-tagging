...
(function(skills, utility, locStorage, txtToList, hScroll, tagBox) {
    ...
    const _getInactivePills = function() {
        return _getPills($("." + ref.skillBox).not("." + ref.active));
    };
    ...
    const initAutoComplete = function () {
        var _config = {
            elem: $(ref.skillFilter),
            dispProp: "name",
            val: "id",
            onSelect: onSelect
        };
        acForSkillSearch = new TSSuggest(_config);
        acForSkillSearch.init();
        acForSkillSearch.data = removedSkills;
    };

    const onSelect = function () {
        const elm = $(ref.skillFilter);
        const pillVal = elm.data("itemId");
        elm.val("");
        const idx = removedSkills.findIndex((i) => {
            return i.id === pillVal;
        });
        if (idx > -1) {
            removedSkills.splice(idx, 1);
            makePills(true);
            _showTagItems();
        } else {
            console.warn("Selected itemwas not found in removed skills list:", removedSkills);
        }
    };
...
})(ProjectNameSpace.SkillFilter,
ProjectNameSpace.TSAutoComplete);
